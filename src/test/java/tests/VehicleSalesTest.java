...
public class VehicleSalesTest {
    ...
    public void testVehicleSales() throws Exception {
        String actual = salesPage.getValueByIndex(5); // Adjust index as needed
        ...
        System.out.println("vehicleSales in App: " + actual + ", vehicleSales in DB: " + expected);
        String status = actual.equals(expected) ? "pass" : "fail";
        TestResultLogger.log("Vehicle Sales", expected, actual, status);
        Assert.assertEquals(actual, expected, "vehicleSales");
    }
} 